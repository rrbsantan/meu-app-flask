...
def dashboard():
    ...
    vendedor_filtro = request.args.get('vendedor', '').strip().lower()  # Novo campo para filtro
    arquivos = []
    coordenadores = []
        ...
        if data and '-' in data:
            ...
            if os.path.exists(pasta_data):
                for pasta in pastas:
                    ...
                    if os.path.exists(coord_path):
                        arquivos += _listar_arquivos(coord_path)
    # Filtra os arquivos pelo nome do vendedor, se informado
    if vendedor_filtro:
        arquivos = [a for a in arquivos if vendedor_filtro in a.get('nome', '').lower()]
    return render_template('dashboard.html', tipo=tipo, pastas=pastas, data=data if data else '', arquivos=arquivos, coordenadores=coordenadores, coord_selecionado=coord_selecionado, dias_com_arquivos=list(dias_com_arquivos), vendedor_filtro=request.args.get('vendedor', ''))
...
def _listar_arquivos(coord_path):
    ...
    for v in vendedores:
        card = {}
        # Extrai primeiro e último nome, removendo underscores
        partes_nome = v.split('_')
        if len(partes_nome) > 1:
            card['nome'] = f"{partes_nome[0]} {partes_nome[-1]}"
        else:
            card['nome'] = v
